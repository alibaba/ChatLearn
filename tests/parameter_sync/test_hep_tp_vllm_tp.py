...
Current test case: 
(dst_ep, dst_tp, dst_pp, src_ep, src_tp, src_pp) = (1, 4, 1, 1, 4, 1).
"""
...
class TestTorchModule(TorchModule):
    ...
    def get_local_param_ranks(self):
        dp_group = [self._get_rank(), int((self._get_rank() + 4) % 8)]
        dp_group.sort()
        return dp_group, int(self._get_rank() // 4)
...
def test_hep_tp_vllm_tp_dst_ep1_tp4_pp1_src_ep1_tp4_pp1():
...
TEST_CASE = [test_hep_tp_vllm_tp_dst_ep1_tp4_pp1_src_ep1_tp4_pp1]
