...
class MegatronModelArchitectureConfig(BaseConfig):
    attention_dropout: float = field(
        default=0.0,
        metadata={"help": "Post attention dropout probability."}
    )
    ...
    hidden_size: int = field(
        default=MISSING,
        metadata={"help": "Tansformer hidden size."}
    )
    ...
    num_query_groups: int = field(
        default=MISSING,
        metadata={"help": "num query groups"}
    )
    ...
    norm_epsilon: float = field(
        default=1e-6,
        metadata={"help": "Epsilon for layer norm and RMS norm."}
    )
    position_embedding_type: str = field(
        default="rope",
        metadata={"help": "Position embedding type."}
    )
    add_qkv_bias: bool = field(
        default=MISSING,
        metadata={"help": "Enable bias only in the QKV linear layers"}
    )
    add_bias_linear: bool = field(
        default=MISSING,
        metadata={"help": "Whether bias in the linear layers"}
    )
    ...
    group_query_attention: bool = field(
        default=MISSING,
        metadata={"help": "Use group-query attention."}
    )
    ...
    tokenizer_type: str = field(
        ...
        metadata={"help": "What type of tokenizer to use."}   
    )
    ...
    extra_vocab_size: int = field(
        default=MISSING,
        metadata={"help": "config vocab_size - tokenizer vocab_size"}
    )
    # debug
...
class MegatronTrainConfig(BaseConfig):
    load: str = field(
        default=MISSING,
        metadata={"help": "path to train model"}
    )
    save: str = field(
        default=MISSING,
    ...
    clip_grad: float = field(
        default=1.0,
        metadata={"help": "Gradient clipping based on global L2 norm."} 
    )
    bf16: bool = field(
        default=True,
        metadata={"help": "Run model in bfloat16 mode."} 
    )
    use_checkpoint_opt_param_scheduler: bool = field(
        default=True,
        metadata={"help": "Use checkpoint to set the values of the scheduler \
    ...
    adam_beta1: float = field(
        default=0.9,
        metadata={"help": "First coefficient for computing running averages of gradient and its square"}
    )
    adam_beta2: float = field(
        default=0.95,
        metadata={"help": "Second coefficient for computing running averages of gradient and its square"}  
    )
    recompute_granularity: str = field(
        default="selective",
        metadata={"help": "Checkpoint activations to allow for training with larger models, sequences, and batch sizes. \
    ...
    finetune: bool = field(
        default=True,
        metadata={"help": "Load model for finetuning. Do not load optimizer or rng state from checkpoint and set iteration to 0."}
    )
    lr: float = field(
        default=2e-6,
    ...
    min_lr: float = field(
        default=0,
        metadata={"help": "Minimum value for learning rate."}
    )
...
@dataclass
class MegatronPolicyTrainerConfig(BaseModelConfig, MegatronModelArchitectureConfig, MegatronTrainConfig):
    """PolicyTrainerConfig"""

    pos_clip_ratio: float = field(
        default=0.2
    )
    neg_clip_ratio: float = field(
        default=0.2
    )
    diff_clip_ratio: float = field(
        default=10
    )
    final_clip_ratio: float = field(
        default=3
    )
