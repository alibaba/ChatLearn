...
class MathEvalAgentGraph(BaseAgentGraph):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(agent_name, llm, tokenizer, **kwargs)
        self.build_graph()
    ...
    def build_graph(self) -> StateGraph:
        ...
        async def tool_node(state: AgentState, config: RunnableConfig):
            ...
            for tool_call in state["messages"][-1].tool_calls:
                ...
                args['gt'] = config['configurable']['gt']
        ...
        def should_continue(state: AgentState):
            ...
            if not last_message.tool_calls or len(messages)>=10:
                return "end"
    ...
    async def run(self, messages, sampling_params: dict[str, Any], **kwargs) -> AgentGraphOutput:
        ...
        # output = self.convert_agent_graph_output(output)
        return output
...
if __name__ == "__main__":
    ...
    agentgraph = MathEvalAgentGraph(agent_name="debughh", llm=llm, tokenizer=tokenizer)
    system_prompt = (
                    "You are a math expert. You are given a question and you need to solve it step by step. "
                    "Reasoning step by step before any tool call. "
                    "You should use the `mathlighteval_reward` tool after step by step solving the question, "
    ...
    out = asyncio.run(agentgraph.run(messages=messages, sampling_params=sampling_params, gt="8"))
    ...
    # out = asyncio.run(agentgraph.run(messages=messages, sampling_params=sampling_params, gt="-\\frac{1}{8}"))
    ...
    # out = asyncio.run(agentgraph.run(messages=messages, sampling_params=sampling_params, gt="40"))
