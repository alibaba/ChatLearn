...
from utils.utils import tensorboard_scalar_dict, read_latest_ppo_iter
...
class RewardModelMegatronInference(RLHFMegatronModule):
    ...
    def setup(self):
        ...
        # TODO tianhang replace this with a get("iteration")
        ...
        if self.args.continue_train:
            ...
            print(f"not continue train load iter: {self.args.iteration_for_log}")
        ...
        self.run_name = f"reward module run-{timestamp}"
    ...
    def model_provider(self, pre_process=True, post_process=True):
        ...
        if self.reward_type == "ziya":
            from models.ziya_reward_model import RewardModel
        else:
            from megatron.model.llama_reward_model import RewardModel
    ...
    def get_math_matching_reward(self, str_prompt, str_response, training_math_golden_reg):
        ...
        print(f"for prompt: {str_prompt}, model response: {str_response}, gold_ans: {gold_ans}, pred_ans: {pred_ans}")

        if pred_ans != INVALID_ANS and abs(float(pred_ans) - float(gold_ans)) < 1e-4:
            score = 1.0
        else:
            score = -1.0
        print(f"thus score is : {score}")
        return score
    ...
    def get_all_rewards(self, action_starts, action_ends, loss_mask, list_strs: List[List[str]], logprobs,
        ...
        # TODO tianhang hack random reward model
        # scores = torch.ones(n, 1, device=loss_mask.device)
        print(f"raw scores: {scores}")
        if self.args.ngram_coef > 0:

            # Get the batch.
            # timers('ngram reward').start()
            ...
            print(f"rewards/ngram_rewards {ngram_rewards.mean()}")
            # stats["rewards/ngram_rewards_max"] = np.max(ngram_rewards)
            # stats["rewards/ngram_rewards_min"] = np.min(ngram_rewards)
            self.per_episode_metrics["rewards/ngram_rewards"].update(ngram_rewards)
            # timers('ngram reward').stop()

            # stats["timer/ngram_time"] = timers.timers['ngram reward'].elapsed(
            #     reset=False) * 1000.0

            # timers.log(['ngram reward'])
        ...
        print(f"raw scores * 2 because rw model use pairs: {scores}")
        ...
        # TODO tianhang collect accross devices to support dp
        ...
        kl_rewards_for_log = [rs[action_starts[ix] - 1: action_ends[ix] - 1] / (action_ends[ix] - action_starts[ix]) for
                              ix, rs in
                              enumerate(rewards)]
        kl_rw_sums = torch.tensor([rr.sum() for rr in kl_rewards_for_log], dtype=torch.float32,
                                  device=torch.cuda.current_device())

        print(f"rewards/klrewards {kl_rw_sums.mean()}")
        ...
        if self.args.lm_coef > 0:
            ...
            # -1 because logprobs are logprobs of action_id[1:]!!! so it's already shifted right, to get logprob of first action, we need -1
            lm_rewards_for_log = [rs[action_starts[ix] - 1: action_ends[ix] - 1] / (action_ends[ix] - action_starts[ix])
                                  for ix, rs
                                  in
                                  enumerate(lm_reward)]
            ...
            print(f"rewards/lmrewards {lm_rewards_for_log_sums.mean()}")
    ...
    def forward_step(self, *data):
        ...
        RLHF calling
        rlhf framework source: reward_output = self.reward.forward_step(policy_output[0], ref_output[0], old_values[0])
        ...
        # TODO tianhang check same name will get overriden
        merged_data = {}
        for batch in data:
            merged_data.update(batch)
        # print("in reward ======== keys", merged_data.keys(), flush=True)
        loss_mask = to_device("cuda", merged_data["loss_mask"])
        no_padded_query_ids = to_device("cuda", merged_data["no_padded_query_ids"])
        all_tokens_right_padded = to_device("cuda", merged_data["all_tokens"])
        str_prompts = merged_data["str_prompts"]
        str_outputs = merged_data["str_outputs"]
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == (
                ...
                save_list_str(list_strs, self.args.iteration_for_log)

        # list_strs = [['sada asjdkha dfnkalbkfd', 'sa sajidha ahkjdfa jsdkaj'], ['sada asjdkha dfnsda sdkalbkfd', 'sa sajidha ahkjass s sdfa jsdkaj']]

        old_value = to_device("cuda", merged_data["old_values"])
        ref_logprobs = to_device("cuda", merged_data["ref_logprobs"])
        logprobs = to_device("cuda", merged_data["logprobs"])
        ...
        if self.args.fix_kl_coef:
            ...
            if kl_coef is None:
                kl_coef = self.args.init_kl_coef
        print(f"in reward model get {kl_coef} ===========")
        ...
        values = old_value.cpu()[:, :-1]
        ref_logprobs = ref_logprobs.cpu()
        logprobs = logprobs.cpu()
        all_tokens_right_padded = all_tokens_right_padded.cpu()
        ...
        for ix, rs in enumerate(starts):
            ...
            print(f"action size {ix} is: {ends[ix] - starts[ix]}")
        ...
        print(f"inference_scores/running_mean {self.running.mean}")
        print(f"inference_scores/running_std {self.running.std}")
        # print(f"all rewards: {all_rewards}")

        # start = query_tensors.shape[1] - 1 is because logprobs is already shifted right by 1. (gather(logprob, token_ids[:, 1:])
        #  ) thus query_tensors.shape[1] - 1 is exactly query_tensors.shape[1]'s value
        ...
        self.log_each_step()
        # TODO tianhang change of key names

        les = [len(j) for j in all_rewards]
        print(f"maxrewardlen {max(les)}")
    ...
    def log_each_step(self):
        ...
        print(f"score only/running_mean {self.running.mean}")
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == (
                ...
                # actual log
                print(f"%%%%%%%%%%" * 10)
                print(f"reward stats: {stats_episode} iter: {self.args.iteration_for_log}")
                tensorboard_scalar_dict(writer, prefix=f"rewards_each/replica_id{self.replica_id}",
                                        global_step=self.args.iteration_for_log,
    ...
    def eval_forward(self, policy_res: dict):
        ...
        if get_args().do_math_eval:
            ...
            print(f"eval rewards/math_rewards {reward_model_scores.mean()}")

        else:
            reward_model_scores = self.get_raw_reward(list_strs).view(-1, 1)
        # scores = self.get_raw_reward(list_strs).view(-1, 1)
        # TODO tianhang hack random reward model
        # reward_model_scores = torch.ones(len(policy_res["str_prompts"]), 1, device=policy_res["all_tokens"].device)
    ...
    def forward_step_pipeline(self, list_strs):
        ...
        self.model.eval()
        if self.reward_type == "ziya":
            from models.ziya_reward_model import batch_padded_tokenize_data
        else:
            from megatron.model.llama_reward_model import batch_padded_tokenize_data
    ...
    def get_raw_reward(self, list_strs: List[List[str]]):
        ...
        exp_score_time = time()
        # all_reward_tokens, loss_masks = self.reward_tokenize(str_samples)
        # #
        scores = self.forward_step_pipeline(list_strs)
        # minus the sft baseline
        #
        scores -= self.reward_bias
        # scores = all_reward_tokens.float().mean(dim=1) / 1000
        # print(f"get rrr {scores}")
        self.stats["time/exp_score"] = time() - exp_score_time

        print(f"rewards/reward_model_scores mean {scores.mean()}")
        ...
        print(f"rewards/normalized_clip_scores mean {scores.mean()}")
